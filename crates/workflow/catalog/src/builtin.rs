...
pub fn builtin_default_source_ref() -> WorkflowSource {
    WorkflowSource {
...
pub(crate) fn load_builtin_source_ref(source_ref: &WorkflowSource) -> Result<LoadedWorkflowSource> {
    if source_ref.id == DEFAULT_WORKFLOW_ID
        && source_ref.location.entry == Path::new(DEFAULT_WORKFLOW_ENTRY)
    {
...
mod tests {
    ...
    fn builtin_catalog_entry_is_available() {
        ...
        assert_eq!(
            built_in.source_ref.location.entry,
            Path::new(DEFAULT_WORKFLOW_ENTRY)
        );
        assert_eq!(built_in.source_ref.location.root, None);
