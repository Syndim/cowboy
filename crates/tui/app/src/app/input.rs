...
pub(super) fn handle_mouse_event(
    ...
    match event.kind {
        ...
        MouseEventKind::ScrollUp
            if layout.composer.contains(position) && state.composer_accepts_submit() =>
        {
...
mod tests {
    ...
    #[tokio::test]
    async fn composer_wheel_changes_only_guarded_input_history() {
        let mut state = test_state();
        seed_history(&mut state);
        assert!(state.scroll_events_up());
        let offset = state.scroll_offset();
        let layout = AppLayout::new(Rect::new(0, 0, 80, 20), &state);
        let point = (layout.composer.x, layout.composer.y);
        ...
        assert_eq!(state.input(), "from history");
        assert_eq!(state.scroll_offset(), offset);
