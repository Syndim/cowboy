...
Review the RCA document, plan document, TODO checklist, local test results, and implementation for correctness, scope control, maintainability, and verification. Evaluate revised work against the complete stage-labeled user feedback history in addition to repository rules, document constraints, and test or validation evidence. Keep raw user feedback distinct from reviewer-generated feedback. Verify each checked TODO item is actually complete, and do not approve while required TODO items are unchecked or falsely checked. For bug fixes, verify the investigator-added repro test still represents the original issue and passes because product code was fixed, not because the test was weakened. Give specific actionable feedback otherwise.]],
  agent = "reviewer",
})
