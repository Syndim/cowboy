...
return function(roles, opts)
  ...
  review.run = function(ctx)
    local prompt, task, errors = context.build_agent_contract(ctx, "blocker_review", {
...
For both statuses, set `blocker_reason` to the evidence-based recoverability analysis. For `recoverable`, also return a non-empty `changes_needed` array and non-empty `change_context` containing only the recovery actions and necessary blocker context. Do not infer facts from generic feedback or body prose beyond the named `blocker_statement`. Preserve every source-specific command and evidence array with semantic deep equality and unchanged array order. Preserve `blocker_statement`, `blocked_from_step`, `blocked_from_status`, `goal`, `validation`, `work_dir`, `plan_doc`, `validation_doc`, `rca_doc`, `repro_test`, and `files` exactly in output fields when present.]],
    })
